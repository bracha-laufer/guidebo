...
class ADULT(data.Dataset):
    ...
    def __init__(self, root="../data/adult", split="train", sensible_attribute="gender", random_state=None, seed=1, **kwargs):
        assert split in ["train", "val", "test", "cal"]
        ...
        x_train, x_test, y_train, y_test, s_train, s_test = train_test_split(x, y, s, test_size=.2, random_state=seed)
        x_train, x_val, y_train, y_val, s_train, s_val = train_test_split(x_train, y_train, s_train, test_size=.125, random_state=seed)
        # add cal split
        x_cal, x_test, y_cal, y_test, s_cal, s_test = train_test_split(x_test, y_test, s_test, test_size=.5,random_state=random_state)
