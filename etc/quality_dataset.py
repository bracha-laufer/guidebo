...
class QualityDataset(Dataset):
    ...
    def __init__(self):
        dataset = [json.loads(j) for j in open('../data/quality/QuALITY.v1.0.1.htmlstripped.train').read().split('\n') if len(j) > 0]
        dataset += [json.loads(j) for j in open('../data/quality/QuALITY.v1.0.1.htmlstripped.dev').read().split('\n') if len(j) > 0]
        dataset += [json.loads(j) for j in open('../data/quality/QuALITY.v1.0.1.htmlstripped.test').read().split('\n') if len(j) > 0]
        self.n_timesteps = 10
        self.num_classes = 4
        X = []
        y = []
